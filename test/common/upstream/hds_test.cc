...
TEST_F(HdsTest, TestSendResponseWithHealthMetadata) {
  ...
  auto& host =
      hds_delegate_->hdsClusters()[0]->prioritySet().hostSetsPerPriority()[0]->hosts()[0];
  ...
  EXPECT_EQ(
      structured_endpoint.health_metadata().fields().at("http_status_code").number_value(), 200.0);
}
